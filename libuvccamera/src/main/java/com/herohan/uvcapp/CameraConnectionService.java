...
//            return USBMonitor.getDeviceKey(device);
        ...
        private ICameraHelper.StateCallback getStateCallback() {
            if (mWeakStateCallback != null) {
                return mWeakStateCallback.get();
            }
            return null;
        }
        //********************************************************************************
...
//            Watchdog.getInstance().removeThread(mListenerHandler);
        ...
        private class MyOnDeviceConnectListener implements OnDeviceConnectListener {
            ...
            public void onAttach(final UsbDevice device) {
                ...
                ICameraHelper.StateCallback callback = getStateCallback();
                if (callback != null) {
                    try {
            ...
            public void onDeviceOpen(final UsbDevice device, final UsbControlBlock ctrlBlock, final boolean createNew) {
                ...
                if (callback != null) {
                    try {
                        callback.onDeviceOpen(device, createNew);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
