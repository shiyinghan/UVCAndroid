...
#define DESC_VENDOR_ID				"vendorId"
#define DESC_PRODUCT_ID				"productId"
#define DESC_SERIALNUMBER			"serialNumber"
#define DESC_MANUFACTURE			"manufactureName"
...
#define INTERFACE_TYPE_VIDEO_STREAM	"videoStreaming"
#define INTERFACE_TYPE_AUDIO_STREAM	"audioStreaming"
#define INTERFACE_INDEX				INDEX
#define INTERFACE_ENDPOINT_ADDRESS		"endpointAddress"

#define FORMATS						"formats"
#define FORMAT_INDEX				INDEX
#define FORMAT_NAME					"format"
#define FORMAT_DESC_SUBTYPE				SUBTYPE
...
#define FORMAT_ASPECT_RATIO_X		"aspectRatioX"
#define FORMAT_ASPECT_RATIO_Y		"aspectRatioY"
#define FORMAT_INTERLACE_FLAGS		"interlaceFlags"
#define FORMAT_COPY_PROTECT			"copyProtect"
#define FORMAT_FRAME_DESCRIPTORS		"frameDescriptors"
...
#define FRAME_FRAME_BUFFER_SIZE_MAX	"maxFrameBufferSize"
...
static void writerFormat(Writer<StringBuffer> &writer, uvc_format_desc_t *fmt_desc) {
	...
	writer.StartObject();
	{
		...
		write(writer, FORMAT_ASPECT_RATIO_X, fmt_desc->bAspectRatioX);
		write(writer, FORMAT_ASPECT_RATIO_Y, fmt_desc->bAspectRatioY);
		write(writer, FORMAT_INTERLACE_FLAGS, fmt_desc->bmInterlaceFlags);
		write(writer, FORMAT_COPY_PROTECT, fmt_desc->bCopyProtect);

		writer.String(FORMAT_FRAME_DESCRIPTORS);
		writer.StartArray();
		...
		writer.EndArray();	// end of FORMAT_FRAME_DESCRIPTORS
	}
				...
				writer.StartObject();
				{
					...
					writer.StartArray();
					{
						...
						DL_FOREACH(deviceHandle->info->stream_ifs, stream_if)
						{
							...
							writer.StartObject();
							{
								write(writer, INTERFACE_TYPE, INTERFACE_TYPE_VIDEO_STREAM);
								write(writer, INTERFACE_INDEX, stream_idx);
								write(writer, INTERFACE_ENDPOINT_ADDRESS, stream_if->bEndpointAddress);
...
char *UVCDiags::getSupportedFormats(const uvc_device_handle_t *deviceHandle) {
	StringBuffer buffer;
	Writer<StringBuffer> writer(buffer);
	char buf[256];
