...
// Convert ABGR to YUY2.
LIBYUV_API
int ABGRToYUY2(const uint8_t* src_abgr,
               int src_stride_abgr,
               uint8_t* dst_yuy2,
               int dst_stride_yuy2,
               int width,
               int height) {
  int y;
  void (*ABGRToUVRow)(const uint8_t* src_abgr, int src_stride_abgr,
                      uint8_t* dst_u, uint8_t* dst_v, int width) =
  ABGRToUVRow_C;
  void (*ABGRToYRow)(const uint8_t* src_abgr, uint8_t* dst_y, int width) =
  ABGRToYRow_C;
  ...
  if (!src_abgr || !dst_yuy2 || width <= 0 || height == 0) {
    return -1;
  }
  // Negative height means invert the image.
  if (height < 0) {
  ...
  // Coalesce rows.
  if (src_stride_abgr == width * 4 && dst_stride_yuy2 == width * 2) {
    width *= height;
    height = 1;
    src_stride_abgr = dst_stride_yuy2 = 0;
  }
#if defined(HAS_ABGRTOYROW_SSSE3)
      if (TestCpuFlag(kCpuHasSSSE3)) {
        ABGRToYRow = ABGRToYRow_Any_SSSE3;
...
#endif
#if defined(HAS_ABGRTOYROW_LASX) && defined(HAS_ABGRTOUVROW_LASX)
      if (TestCpuFlag(kCpuHasLASX)) {
    ABGRToYRow = ABGRToYRow_Any_LASX;
    ABGRToUVRow = ABGRToUVRow_Any_LASX;
    if (IS_ALIGNED(width, 32)) {
      ABGRToYRow = ABGRToYRow_LASX;
      ABGRToUVRow = ABGRToUVRow_LASX;
    }
  }
#endif
#if defined(HAS_I422TOYUY2ROW_SSE2)
...

  {
    ...
    for (y = 0; y < height; ++y) {
      ABGRToUVRow(src_abgr, 0, row_u, row_v, width);
      ABGRToYRow(src_abgr, row_y, width);
      I422ToYUY2Row(row_y, row_u, row_v, dst_yuy2, width);
...
LIBYUV_API
int ABGRToRGB565(const uint8_t* src_abgr,
                 int src_stride_abgr,
                 uint8_t* dst_rgb565,
                 int dst_stride_rgb565,
                 int width,
                 int height) {
      int y;
      void (*ABGRToRGB565Row)(const uint8_t* src_abgr, uint8_t* dst_rgb,
                              int width) = ABGRToRGB565Row_C;
      if (!src_abgr || !dst_rgb565 || width <= 0 || height == 0) {
        return -1;
      }
      if (height < 0) {
        height = -height;
      ...
      // Coalesce rows.
      if (src_stride_abgr == width * 4 && dst_stride_rgb565 == width * 2) {
        width *= height;
        height = 1;
        src_stride_abgr = dst_stride_rgb565 = 0;
      }
#if defined(HAS_ABGRTORGB565ROW_SSE2)
      if (TestCpuFlag(kCpuHasSSE2)) {
        ABGRToRGB565Row = ABGRToRGB565Row_Any_SSE2;
        if (IS_ALIGNED(width, 4)) {
          ABGRToRGB565Row = ABGRToRGB565Row_SSE2;
        }
      }
#endif
#if defined(HAS_ABGRTORGB565ROW_AVX2)
      if (TestCpuFlag(kCpuHasAVX2)) {
    ABGRToRGB565Row = ABGRToRGB565Row_Any_AVX2;
    if (IS_ALIGNED(width, 8)) {
      ABGRToRGB565Row = ABGRToRGB565Row_AVX2;
    }
  }
#endif
#if defined(HAS_ABGRTORGB565ROW_NEON)
      if (TestCpuFlag(kCpuHasNEON)) {
    ABGRToRGB565Row = ABGRToRGB565Row_Any_NEON;
    if (IS_ALIGNED(width, 8)) {
      ABGRToRGB565Row = ABGRToRGB565Row_NEON;
    }
  }
#endif
#if defined(HAS_ABGRTORGB565ROW_MSA)
      if (TestCpuFlag(kCpuHasMSA)) {
    ABGRToRGB565Row = ABGRToRGB565Row_Any_MSA;
    if (IS_ALIGNED(width, 8)) {
      ABGRToRGB565Row = ABGRToRGB565Row_MSA;
    }
  }
#endif
#if defined(HAS_ABGRTORGB565ROW_LASX)
      if (TestCpuFlag(kCpuHasLASX)) {
    ABGRToRGB565Row = ABGRToRGB565Row_Any_LASX;
    if (IS_ALIGNED(width, 16)) {
      ABGRToRGB565Row = ABGRToRGB565Row_LASX;
    }
  }
#endif

  for (y = 0; y < height; ++y) {
    ABGRToRGB565Row(src_abgr, dst_rgb565, width);
    src_abgr += src_stride_abgr;
    dst_rgb565 += dst_stride_rgb565;
  }
  return 0;
