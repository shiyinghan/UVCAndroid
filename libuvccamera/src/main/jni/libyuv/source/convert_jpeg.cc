...
static void JpegI420ToABGR(void* opaque,
                           const uint8_t* const* data,
                           const int* strides,
                           int rows) {
  ABGRBuffers* dest = (ABGRBuffers*)(opaque);
  I420ToABGR(data[0], strides[0], data[1], strides[1], data[2], strides[2],
             dest->abgr, dest->abgr_stride, dest->w, rows);
  dest->abgr += rows * dest->abgr_stride;
  dest->h -= rows;
}

static void JpegI422ToABGR(void* opaque,
                           const uint8_t* const* data,
                           const int* strides,
                           int rows) {
  ABGRBuffers* dest = (ABGRBuffers*)(opaque);
  I422ToABGR(data[0], strides[0], data[1], strides[1], data[2], strides[2],
             dest->abgr, dest->abgr_stride, dest->w, rows);
  dest->abgr += rows * dest->abgr_stride;
  dest->h -= rows;
}

static void JpegI444ToABGR(void* opaque,
                           const uint8_t* const* data,
                           const int* strides,
                           int rows) {
  ABGRBuffers* dest = (ABGRBuffers*)(opaque);
  I444ToABGR(data[0], strides[0], data[1], strides[1], data[2], strides[2],
             dest->abgr, dest->abgr_stride, dest->w, rows);
  dest->abgr += rows * dest->abgr_stride;
  dest->h -= rows;
}

static void JpegI400ToABGR(void* opaque,
                           const uint8_t* const* data,
                           const int* strides,
                           int rows) {
  ABGRBuffers* dest = (ABGRBuffers*)(opaque);
  //in gray image: R = G = B
  I400ToARGB(data[0], strides[0], dest->abgr, dest->abgr_stride, dest->w, rows);
  dest->abgr += rows * dest->abgr_stride;
  dest->h -= rows;
}

// MJPG (Motion JPeg) to ABGR
// TODO(fbarchard): review src_width and src_height requirement. dst_width and
// dst_height may be enough.
LIBYUV_API
int MJPGToABGR(const uint8_t* src_mjpg,
               size_t src_size_mjpg,
               uint8_t* dst_abgr,
               int dst_stride_abgr,
               int src_width,
  ...
  if (ret) {
    ABGRBuffers bufs = {dst_abgr, dst_stride_abgr, dst_width, dst_height};
    // YUV420
    if (mjpeg_decoder.GetColorSpace() == MJpegDecoder::kColorSpaceYCbCr &&
        mjpeg_decoder.GetNumComponents() == 3 &&
        mjpeg_decoder.GetVertSampFactor(0) == 2 &&
        ...
        mjpeg_decoder.GetVertSampFactor(2) == 1 &&
        mjpeg_decoder.GetHorizSampFactor(2) == 1) {
      ret = mjpeg_decoder.DecodeToCallback(&JpegI420ToABGR, &bufs, dst_width,
                                           dst_height);
      // YUV422
    } else if (mjpeg_decoder.GetColorSpace() ==
               MJpegDecoder::kColorSpaceYCbCr &&
               ...
               mjpeg_decoder.GetVertSampFactor(2) == 1 &&
               mjpeg_decoder.GetHorizSampFactor(2) == 1) {
      ret = mjpeg_decoder.DecodeToCallback(&JpegI422ToABGR, &bufs, dst_width,
                                           dst_height);
      // YUV444
    } else if (mjpeg_decoder.GetColorSpace() ==
               MJpegDecoder::kColorSpaceYCbCr &&
               ...
               mjpeg_decoder.GetVertSampFactor(2) == 1 &&
               mjpeg_decoder.GetHorizSampFactor(2) == 1) {
      ret = mjpeg_decoder.DecodeToCallback(&JpegI444ToABGR, &bufs, dst_width,
                                           dst_height);
      // YUV400
    } else if (mjpeg_decoder.GetColorSpace() ==
               MJpegDecoder::kColorSpaceGrayscale &&
               ...
               mjpeg_decoder.GetVertSampFactor(0) == 1 &&
               mjpeg_decoder.GetHorizSampFactor(0) == 1) {
      ret = mjpeg_decoder.DecodeToCallback(&JpegI400ToABGR, &bufs, dst_width,
                                           dst_height);
    } else {
      // TODO(fbarchard): Implement conversion for any other
      // colorspace/subsample factors that occur in practice. ERROR: Unable to
