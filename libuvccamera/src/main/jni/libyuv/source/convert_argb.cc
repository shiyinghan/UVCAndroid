...
// Convert YUY2 to ABGR.
LIBYUV_API
int YUY2ToABGR(const uint8_t* src_yuy2,
               int src_stride_yuy2,
               uint8_t* dst_abgr,
               int dst_stride_abgr,
               int width,
               int height) {
  int y;
  void (*YUY2ToABGRRow)(const uint8_t* src_yuy2, uint8_t* dst_abgr,
                        const struct YuvConstants* yuvconstants, int width) =
  YUY2ToABGRRow_C;
  if (!src_yuy2 || !dst_abgr || width <= 0 || height == 0) {
    return -1;
  }
  // Negative height means invert the image.
  if (height < 0) {
  ...
  // Coalesce rows.
  if (src_stride_yuy2 == width * 2 && dst_stride_abgr == width * 4) {
    width *= height;
    height = 1;
    src_stride_yuy2 = dst_stride_abgr = 0;
  }
#if defined(HAS_YUY2TOABGRROW_SSSE3)
      if (TestCpuFlag(kCpuHasSSSE3)) {
        YUY2ToABGRRow = YUY2ToABGRRow_Any_SSSE3;
        if (IS_ALIGNED(width, 16)) {
          YUY2ToABGRRow = YUY2ToABGRRow_SSSE3;
        }
      }
#endif
#if defined(HAS_YUY2TOABGRROW_AVX2)
      if (TestCpuFlag(kCpuHasAVX2)) {
        YUY2ToABGRRow = YUY2ToABGRRow_Any_AVX2;
        if (IS_ALIGNED(width, 32)) {
          YUY2ToABGRRow = YUY2ToABGRRow_AVX2;
        }
      }
#endif
#if defined(HAS_YUY2TOABGRROW_NEON)
  if (TestCpuFlag(kCpuHasNEON)) {
    YUY2ToABGRRow = YUY2ToABGRRow_Any_NEON;
    if (IS_ALIGNED(width, 8)) {
      YUY2ToABGRRow = YUY2ToABGRRow_NEON;
    }
  }
#endif
#if defined(HAS_YUY2TOABGRROW_MSA)
      if (TestCpuFlag(kCpuHasMSA)) {
    YUY2ToABGRRow = YUY2ToABGRRow_Any_MSA;
    if (IS_ALIGNED(width, 8)) {
      YUY2ToABGRRow = YUY2ToABGRRow_MSA;
    }
  }
#endif
#if defined(HAS_YUY2TOABGRROW_LSX)
      if (TestCpuFlag(kCpuHasLSX)) {
    YUY2ToABGRRow = YUY2ToABGRRow_Any_LSX;
    if (IS_ALIGNED(width, 8)) {
      YUY2ToABGRRow = YUY2ToABGRRow_LSX;
    }
  }
#endif
  for (y = 0; y < height; ++y) {
    YUY2ToABGRRow(src_yuy2, dst_abgr, &kYvuI601Constants, width);
    src_yuy2 += src_stride_yuy2;
    dst_abgr += dst_stride_abgr;
  }
  return 0;
